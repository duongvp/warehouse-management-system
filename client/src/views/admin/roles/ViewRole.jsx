...
export default function ViewRole() {
  ...
  useEffect(() => {
    async function getDataImport() {
      const response = await fetch(`http://localhost:5000/api/role/get`);
    ...
    if (confirmStatus) {
        ...
        fetch('http://localhost:5000/api/role/delete/' + idDelete, {
          method: 'DELETE',
        })
        .then(res => {
  ...
  const handleEdit = (id) => {
    navigate(`/admin/member-role/edit/${id}`)
  }
  ...
  const handleSearchData = async() => {
    const response = await fetch(`http://localhost:5000/api/role/get?name=${roleName}`);
    const jsonData = await response.json();
    console.log(jsonData);
    setData(jsonData);
  }
        ...
        <Card extra={"w-full sm:overflow-auto p-4"}>
            <header className="relative flex items-center justify-between">
                <div className="flex text-navy-700 dark:text-white h-full">
                    <SearchField placeholder="Theo tên vai trò..." value={roleName} handleChangeValue={setRoleName}/>
                ...
                <button className='bg-blue-500 text-white text-[13px] p-2 rounded flex items-center' onClick={() => navigate(`/admin/member-role/create-new`)}><MdAdd className='text-[16px] mr-1'/>Thêm mới</button>
            </header>
                ...
                <tbody>
                    ...
                    return (
                        ...
                        {columns.map((cell) => {
                            ...
                            } else if (cell.accessor == 'permissions') {
                                if(Array.isArray(row[cell.accessor])) {
                                    view = (
                                        <div className="flex items-center gap-2  max-w-[380px]">
                                              <p className="text-sm font-bold text-navy-700 dark:text-white w-full truncate">
                                                  {row[cell.accessor]?.map(ele => (<span className='pr-3'>{ele}</span>))}
                                              </p>
                                        </div>
                                    );
                                } else {
                                    view = (
                                        <div className="flex items-center gap-2 text-white max-w-[380px]">
            ...
            <Confirm statusOpenCf={statusOpenCf} setStatusOpenCf={setStatusOpenCf} setConfirmStatus={setConfirmStatus} text={"Bạn chắc chắn muốn xóa vai trò này không?"}/>
        </Card>
    </div>
  )
}
