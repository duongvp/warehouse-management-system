...
export default function ViewCategory() {
  ...
  useEffect(() => {
    ...
    if (confirmStatus) {
        ...
        fetch('http://localhost:5000/api/category/delete/' + idDelete, {
          method: 'DELETE',
        })
        .then(res => {
  ...
  const handleSearchData = async() => {
    const response = await fetch(`http://localhost:5000/api/category/get?name=${categoryName}`);
    const jsonData = await response.json();
        ...
        <Card extra={"w-full sm:overflow-auto p-4"}>
            <header className="relative flex items-center justify-between">
                <div className="flex text-navy-700 dark:text-white h-full">
                    <SearchField placeholder="Theo tên loại hàng..." value={categoryName} handleChangeValue={setCategoryName}/>
                ...
                <button className='bg-blue-500 text-white text-[13px] p-2 rounded flex items-center' onClick={() => navigate(`/admin/category/create-new`)}><MdAdd className='text-[16px] mr-1'/>Thêm mới</button>
            </header>
