...
const CustomerOption= ({ innerRef, innerProps, data }) => (
    <div ref={innerRef} {...innerProps} className="flex py-2 px-3 hover:bg-sky-700">
...
export default function CreateDeliveryVoucher(props) {
  const {context} = props
  ...
  const [customerNeedPay, setCustomerNeedPay] = useState(0);
  ...
  useEffect(() => {
    async function getData() {
      const str = JSON.stringify({proName: '', status: 'true'})
      const urls = [`http://localhost:5000/api/product/get?warehouseId=${currentWH}&page=1&perPage=1000&objQuery=${str}`, "http://localhost:5000/api/customer/get" ]
      ...
      setCustomerOptions(jsonData[1].map(item => ({
        value:item._id,
        label:item.name,
        phone:item.phone,
        id: item._id,
    ...
    if(params.id) {
        const getApiDetail = async (id) => {
            const res =  await fetch(`http://localhost:5000/api/warehouseDelivery/detail/${id}`)
            ...
            setDebtors(dataOrder.debtors)
            setIdDelivery(dataOrder._id)
            setCustomerNeedPay(dataOrder.customerPaid)
            setNote(dataOrder.note || "")
            setDiscount(dataOrder.discountMount)
            setTotalPricePro(dataOrder.totalPrice)
            setCustomerPaid(dataOrder.debtors + dataOrder.customerPaid)
            setSelectedCustomerOption({
                id: dataOrder.customerId._id,
                label: dataOrder.customerId.name,
                phone: dataOrder.customerId.phone,
                value: dataOrder.customerId._id
            })
  ...
  const handleChangeAmountPro = (id, value) => {
    let checkItem = productImport.find(item => item.id === id)
    console.log(checkItem);
    if (Number(value) <= Number(checkItem.instock)) {
  ...
  const handleChangePaidSuppiler =  useCallback((e) => {
    setDebtors(Number(e.target.value) - Number(customerNeedPay))
    setCustomerPaid(e.target.value)
  }, [customerPaid])
  const handleChangeDiscount = useCallback((e) => {
    setCustomerNeedPay(Number(totalPricePro) - Number(e.target.value))
    setDiscount(e.target.value)
  }, [discount])
  const handleSaveData = async () => {
    let url = 'http://localhost:5000/api/warehouseDelivery/create'
    let data = {
        idDelivery,
        productImport,
    ...
    if (params.id) {
        url ='http://localhost:5000/api/warehouseDelivery/edit'
        data = {
            idDelivery,
            discount,
            customerNeedPay,
            ...
            <header className="relative flex items-center justify-between">
                <div className="text-navy-700 dark:text-white h-full w-full text-[14px]">
                    <Select
                        ...
                        onChange={(choice) => setSelectedCustomerOption(choice)}
                        options={customerOptions}
                        components={{ Option: CustomerOption }}
                        name="NCC"
                        isDisabled = {params.id ? true : false}
                        placeholder="Tìm khách hàng theo tên hoặc sdt..."
                    />
                </div>
            </header>
            <div className="text-navy-700 dark:text-white text-[14px] mt-8">
                <div className='grid w-full md:grid-cols-2'>
                    <div><span>Mã phiếu xuất</span></div>
                    <div>
                        <input type="text" className='w-full border-b outline-0' placeholder='Mã phiếu tự động' value={idDelivery} onChange={(e) => setIdDelivery(e.target.value)} disabled = {params.id ? true : false}/>
                    </div>
                </div>   
                ...
                <div className='grid w-full md:grid-cols-2 mt-5'>
                    <div><span>Khách cần trả</span></div>
                    <div className='text-right'>
                        <span>{customerNeedPay}</span>
                    </div>
                </div> 
                <div className='grid w-full md:grid-cols-3 mt-5'>
                    <div className='col-span-2'><span>Tiền khách trả</span></div>
                    <div>
                        <input type="number" className='w-full border-b outline-0 text-right'  value={customerPaid} onChange={handleChangePaidSuppiler}/>
                    </div>
                </div> 
                <div className='grid w-full md:grid-cols-2 mt-5'>
                    <div><span>Tiền khách nợ</span></div>
                    <div className='text-right'>
                        <span>{debtors}</span>
                    </div>
                </div> 
