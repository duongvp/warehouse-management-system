...
const CreateNewUnit = (props) => {
    ...
    useEffect(() => {
        async function getData() {
            if(params.unitId) {
                let urlGetInfoProduct = `http://localhost:5000/api/unit/detail/${params.unitId}`
    ...
    const handleSave = async () => {
        setLoading(true);
        let url = "http://localhost:5000/api/unit/create";
        let method = 'POST';
        let textAlert = 'Thêm mới thành công'
        if (params.unitId) {
            url = `http://localhost:5000/api/unit/edit/${params.unitId}`;
            method = "PUT";
        ...
        <div className='mt-5 grid h-full'>
            <Card extra={"w-full sm:overflow-auto p-4"}>
                ...
                <div className="mt-8 overflow-x-scroll xl:overflow-x-hidden">
                        <InputField label="Tên đơn vị" id="nameUser" placeholder="Nhập tên đơn vị" type="text" value={name} handleChangeValue={setName}/>
