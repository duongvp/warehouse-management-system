...
export default function ViewUnit() {
  ...
  useEffect(() => {
    async function getDataImport() {
      const response = await fetch(`http://localhost:5000/api/unit/get`);
      const jsonData = await response.json();
      setData(jsonData)
    }
    if (confirmStatus) {
        setConfirmStatus(false);
        fetch('http://localhost:5000/api/unit/delete/' + idDelete, {
          method: 'DELETE',
        })
        .then(res => {
  ...
  const handleSearchData = async() => {
    const response = await fetch(`http://localhost:5000/api/unit/get?name=${unitName}`);
    const jsonData = await response.json();
        ...
        <Card extra={"w-full sm:overflow-auto p-4"}>
            <header className="relative flex items-center justify-between">
                <div className="flex text-navy-700 dark:text-white h-full">
                    <SearchField placeholder="Theo đơn vị tính..." value={unitName} handleChangeValue={setUnitName}/>
                ...
                <button className='bg-blue-500 text-white text-[13px] p-2 rounded flex items-center' onClick={() => navigate(`/admin/unit/create-new`)}><MdAdd className='text-[16px] mr-1'/>Thêm mới</button>
            </header>
