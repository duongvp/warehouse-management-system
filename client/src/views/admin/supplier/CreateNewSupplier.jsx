...
const CreateNewSupplier = (props) => {
    ...
    useEffect(() => {
        async function getData() {
            if(params.supplierId) {
                let urlGetInfoProduct = `http://localhost:5000/api/supplier/detail/${params.supplierId}`
    ...
    const handleSave = async () => {
        setLoading(true);
        let url = "http://localhost:5000/api/supplier/create";
        let method = 'POST';
        let textAlert = 'Thêm mới thành công'
        if (params.supplierId) {
            url = `http://localhost:5000/api/supplier/edit/${params.supplierId}`;
            method = "PUT";
        ...
        <div className='mt-5 grid h-full'>
            <Card extra={"w-full sm:overflow-auto p-4"}>
                ...
                <div className="mt-8 overflow-x-scroll xl:overflow-x-hidden">
                        <InputField label="Tên nhà cung cấp" id="nameUser" placeholder="Nhập tên người dùng" type="text" value={name} handleChangeValue={setName}/>
