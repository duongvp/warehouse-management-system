...
export default function ViewCustomer() {
  ...
  useEffect(() => {
    ...
    if (confirmStatus) {
        ...
        fetch('http://localhost:5000/api/customer/delete/' + idDelete, {
          method: 'DELETE',
        })
        .then(res => {
  ...
  const handleSearchData = async() => {
    const response = await fetch(`http://localhost:5000/api/customer/get?name=${customerName}&phone=${customerPhone}`);
    const jsonData = await response.json();
        ...
        <Card extra={"w-full sm:overflow-auto p-4"}>
            <header className="relative flex items-center justify-between">
                <div className="flex text-navy-700 dark:text-white h-full">
                    <SearchField placeholder="Theo tên khách hàng..." value={customerName} handleChangeValue={setCustomerName}/>
                    <SearchField extra = "w-[180px]" type = "number" placeholder="Theo số điện thoại..." value={customerPhone} handleChangeValue={setCustomerPhone}/>
                ...
                <button className='bg-blue-500 text-white text-[13px] p-2 rounded flex items-center' onClick={() => navigate(`/admin/customer/create-new`)}><MdAdd className='text-[16px] mr-1'/>Thêm mới</button>
            </header>
            ...
            <Confirm statusOpenCf={statusOpenCf} setStatusOpenCf={setStatusOpenCf} setConfirmStatus={setConfirmStatus} text={"Bạn chắc chắn muốn xóa khách này này không?"}/>
        </Card>
    </div>
  )
}
