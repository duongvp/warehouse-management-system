...
const CreateNewWh = (props) => {
    ...
    useEffect(() => {
        async function getData() {
            if(params.id) {
                let urlGetInfoProduct = `http://localhost:5000/api/warehouse/detail/${params.id}`
    ...
    const handleSave = async () => {
        setLoading(true);
        let url = "http://localhost:5000/api/warehouse/create";
        let method = 'POST';
        let textAlert = 'Thêm mới thành công'
        if (params.id) {
            url = `http://localhost:5000/api/warehouse/edit/${params.id}`;
            method = "PUT";
        ...
        <div className='mt-5 grid h-full'>
            <Card extra={"w-full sm:overflow-auto p-4"}>
                ...
                <div className="mt-8 overflow-x-scroll xl:overflow-x-hidden">
                        <InputField label="Tên kho hàng" id="nameUser" placeholder="Nhập tên kho hàng" type="text" value={name} handleChangeValue={setName}/>
